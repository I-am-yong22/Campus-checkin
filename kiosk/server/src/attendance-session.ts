...
async function resolvePlatformRule(
  prisma: typeof import('./db.js').prisma,
  date: string,
): Promise<EffectiveAttendanceRule | null> {
  const task = await prisma.attendanceTask.findUnique({
    where: { date },
  });
  if (task?.status === 'PUBLISHED') {
    return ruleFromTask(task);
  }
  const rule = await prisma.platformCheckInRule.findUnique({ where: { id: 1 } });
  if (!rule) return null;
  return ruleFromPlatformRule(rule);
}

export { resolvePlatformRule };
