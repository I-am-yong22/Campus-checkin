...
> Campus face-recognition check-in system for summer school programs

> 最后更新：2026-07-05

暑期学校团体打卡系统：人脸识别签到/签退、请假审批、数据统计、团队管理。三种角色——**普通用户**、**项目负责人**、**管理员**。

> **许可证：** 本项目采用 [PolyForm Noncommercial 1.0.0](LICENSE) 许可证。**禁止未经授权的商业使用。** 如需商用授权，请通过 GitHub Issues 联系作者。
...
种子数据包含 **4 个团队** 与多组测试账号。`npm run seed` 可重复执行以**补齐**团队、成员关系、邀请码等平台数据；**已有账号的密码不会在 update 时重置**（仅 `create` 时写入 `passwordHash`）。

| 角色 | 用户名 | 初始密码 | 说明 |
| --- | --- | --- | --- |
| 管理员 | admin | admin123 | 全平台管理 |
| 项目负责人 | leader | leader123 | 负责「暑期实践一团 + 二团」，可测多团队切换 |
| 项目负责人 | leader2 | leader123 | 负责「创新创业团」 |
| 项目负责人 | leader3 | leader123 | 负责「志愿服务队」 |
| 普通用户 | user | user123 | 学员，仅「暑期实践一团」 |
| 普通用户 | user10 | user123 | 学员，「一团 + 二团」，可测切换 |
| 普通用户 | user2 | user123 | 无团队，可测邀请码入团 |
| 普通用户 | user3~user9 | 123456 | 各团队学员（见下表） |

**团队与邀请码**

| 团队 | 邀请码 | 负责人 | 学员 |
| --- | --- | --- | --- |
| 暑期实践一团 | TEAM01 | leader | user, user10, user3, user4 |
| 暑期实践二团 | TEAM02 | leader | user10, user5, user6 |
| 创新创业团 | TEAM03 | leader2 | user7, user8 |
| 志愿服务队 | TEAM04 | leader3 | user9 |

**测试建议**

- `leader` → 我的团队 → 切换一团/二团 → 成员、出勤、统计、踢人
- `user10` → 我的团队 → 切换一团/二团查看成员
- `user2` → 我的团队 → 加入团队 → 输入 `TEAM01`~`TEAM04`
