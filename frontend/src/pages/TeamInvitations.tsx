...
import { Alert, Button, Card, Divider, Empty, Input, List, Popconfirm, Space, Tag, message } from 'antd';
    ...
    <Card title="团队邀请">
      {hasTeam && isUser && (
        <Alert
          ...
          message={`您当前在「${user?.team?.name || '团队'}」`}
          description="如需更换团队，可先退出当前团队，再通过邀请码或点对点邀请加入新团队。"
          action={
            <Popconfirm
              title="确定退出当前团队？"
            ...
            >
              <Button size="small" danger loading={leaving}>
                退出团队
              </Button>
            </Popconfirm>
          }
        ...
        <>
          <Card type="inner" title="邀请码加入" style={{ marginBottom: 16 }}>
            <Alert
              ...
              showIcon
              style={{ marginBottom: 16 }}
              message="向项目负责人索取 6 位邀请码，预览团队信息并确认后加入。"
            />
              ...
              <List.Item.Meta
                ...
                description={
                  <div>
                    ...
                    {item.team?.description && <div>团队说明：{item.team.description}</div>}
