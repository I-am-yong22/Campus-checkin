...
export default function TeamAttendancePanel({ teamId }: Props) {
  ...
  const columns: ColumnsType<TeamMemberRow> = useMemo(
    () => [
      ...
      {
        title: '签退时间',
        render: (_, r) => (r.checkIn?.checkOutAt ? dayjs(r.checkIn.checkOutAt).format('HH:mm:ss') : '—'),
        width: 100,
      },
      {
        title: '工时',
        render: (_, r) => (r.checkIn?.workMinutes != null ? `${r.checkIn.workMinutes} 分` : '—'),
        width: 80,
      },
      {
        title: '比对距离',
        render: (_, r) => (r.checkIn ? r.checkIn.matchScore?.toFixed(3) : '—'),
        width: 100,
      },
      {
        title: '操作',
    ...
    <div>
      ...
      {data && (
        <Row gutter={16} style={{ marginBottom: 16 }}>
          ...
          <Col xs={12} sm={8} md={4}>
            <Card size="small">
              <Statistic title="在岗" value={data.summary.onDuty ?? 0} valueStyle={{ color: brand.primary }} />
            </Card>
          </Col>
          <Col xs={12} sm={8} md={4}>
            <Card size="small">
              <Statistic title="已完成" value={data.summary.completed ?? 0} valueStyle={{ color: chartColors.success }} />
            </Card>
          </Col>
          <Col xs={12} sm={8} md={4}>
            <Card size="small">
              <Statistic title="迟到" value={data.summary.late} valueStyle={{ color: chartColors.warning }} />
            </Card>
          </Col>
          <Col xs={12} sm={8} md={4}>
            <Card size="small">
              <Statistic title="补签" value={data.summary.makeup} valueStyle={{ color: brand.primary }} />
            </Card>
          </Col>
          <Col xs={12} sm={8} md={4}>
            <Card size="small">
              <Statistic title="请假" value={data.summary.onLeave} valueStyle={{ color: chartColors.secondary }} />
            </Card>
          </Col>
          <Col xs={12} sm={8} md={4}>
            <Card size="small">
      ...
      <Table
        ...
        title={() => (data?.team ? `${data.team.name} · ${data.date} 成员签到` : undefined)}
      />
      ...
      >
        <Form form={makeupForm} layout="vertical">
          ...
          <Form.Item
            name="remark"
            label="补签原因"
            rules={[{ required: true, message: '请填写补签原因' }]}
          >
